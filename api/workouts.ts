...
export const WorkoutsAPI = {
  getWorkoutLogs: async (userId: string, limit = 10, offset = 0): Promise<any[]> => {
    try {
  ...
  getWorkoutDetails: async (workoutLogId: number): Promise<WorkoutExerciseLog[]> => {
    try {
      const { data, error } = await withTimeout(
        supabase
  ...
  getExerciseProgress: async (userId: string, exerciseId: number): Promise<any[]> => {
    try {
  ...
  saveWorkoutSession: async (log: Partial<WorkoutLog>, exercises: Partial<WorkoutExerciseLog>[]) => {
    try {
      // 1. Save main log
